  ...
  return (
    <>
      <div className="fixed top-6 right-6 z-40">
        ...
        >
          <Search className={`w-6 h-6 group-hover:scale-110 transition-transform ${theme === 'dark' ? 'text-white' : 'text-black'}`} />
        </button>
      </div>
          ...
          >
            <div className={`container mx-auto max-w-4xl h-full flex flex-col transition-colors duration-500 ${theme === 'dark' ? 'text-white' : 'text-black'}`}>
              <div className="flex justify-between items-center mb-12">
                ...
                <button 
                  onClick={() => { setIsOpen(false); setResults(null); setQuery(''); playSFX('click'); }}
