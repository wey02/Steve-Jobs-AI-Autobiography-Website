...
export const QuoteGallery: React.FC = () => {
  ...
  const getRandomQuote = () => {
    ...
    setTimeout(() => {
      const filtered = filter ? quotes.filter(q => q.topic === filter) : quotes;
      const otherQuotes = filtered.filter(q => q.text !== currentQuote.text);
      const next = otherQuotes.length > 0 
  ...
  return (
    <section className={`py-24 transition-colors duration-500 ${theme === 'dark' ? 'bg-zinc-900 text-white' : 'bg-zinc-100 text-black'}`}>
        ...
        <div className="flex flex-col md:flex-row justify-between items-end mb-16 gap-8">
          <div>
            ...
            <motion.p 
              ...
              className={`transition-colors duration-500 ${theme === 'dark' ? 'text-gray-400' : 'text-gray-500'}`}
            >
          ...
          <div className="flex flex-wrap gap-2">
            <button 
              onClick={() => { setFilter(null); playSFX('click'); }}
              className={`px-4 py-2 rounded-full text-sm transition-all ${!filter ? (theme === 'dark' ? 'bg-white text-black' : 'bg-black text-white') : (theme === 'dark' ? 'bg-white/5 text-white/60 hover:bg-white/10' : 'bg-black/5 text-black/60 hover:bg-black/10')}`}
            >
              All
            </button>
            {topics.map(topic => (
              <button 
                key={topic}
                onClick={() => { setFilter(topic); playSFX('click'); }}
                className={`px-4 py-2 rounded-full text-sm transition-all ${filter === topic ? (theme === 'dark' ? 'bg-white text-black' : 'bg-black text-white') : (theme === 'dark' ? 'bg-white/5 text-white/60 hover:bg-white/10' : 'bg-black/5 text-black/60 hover:bg-black/10')}`}
              >
                {topic}
              </button>
            ))}
